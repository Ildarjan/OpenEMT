...
| [`lv_04kv_rooftop_pv.json`](lv_04kv_rooftop_pv.json) | **3-ph only** | A 10/0.4 kV Dyn11, 250 kVA transformer supplies a weak 500 m radial LV feeder with twenty single-phase houses. Ten evenly spaced houses have rooftop GFL inverters distributed A/B/C = 4/3/3. Nine use moderate PI gains and operate steadily; inverter #719 at the remote house is initially disconnected, closes at 3.0 s, ramps to 3.5 kW over 150 ms, and has deliberately aggressive active/reactive gains. Its interaction with the feeder and transformer impedance produces growing voltage and P/Q oscillations during the saved 5 s run. The saved plots show the end-of-feeder RMS voltage and inverter #719 power. This is a reduced-order control-instability demonstration: the inverter is an averaged EMF-behind-filter model, without PLL, switching, or LCL resonance. The grounded transformer star point and ground returns represent the neutral; explicit neutral-conductor impedance and neutral displacement are outside the current phase-tap model. Power flow is disabled because positive sequence cannot initialize an unbalanced lateral circuit. |
